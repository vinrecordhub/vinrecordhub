// api/fulfill-order.js — undici proxy version
...
async function verifyPayPalPayment(orderId, expectedAmount) {
  ...
  return isPaid && Math.abs(paidAmount - expectedAmount) < 0.01;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({
      error: 'Failed to deliver report. Contact support@vinrecordhub.com with your order ID.',
      details: err.message,
    });
  }
};
